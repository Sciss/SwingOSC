...
//                repaint();
    ...
    private void stateUpdate() {
        ...
        if( !sv.colrLabel[ NORMAL ].equals( getForeground() )) setForeground( sv.colrLabel[ NORMAL ]);
        if( sv.isClear ) {
            if( getBackground() != null ) setBackground( null );
//        } else if( sv.isGradient ) {
//
        } else {
            if( !getBackground().equals( sv.colrSVBackTop[ NORMAL ])) setBackground( sv.colrSVBackTop[ NORMAL ]);
        }
